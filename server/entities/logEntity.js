...
class LogEntity {
  getDailyLogs(startDate, endDate) {
    // startDate and endDate can be undefined
    return new Promise((resolve, reject) => {
      ...
      if (startDate && endDate) {
        if (
          ...
          console.warn(
            "getDailyLogs (Entity): Received invalid date formats, fetching all logs instead."
          );
  ...
  getWeeklyLogs(startDate, endDate) {
    // startDate and endDate are expected to be valid
    return new Promise((resolve, reject) => {
      const query = `
        SELECT
  ...
  getMonthlyLogs(startDate, endDate) {
    // startDate and endDate are expected to be valid
    return new Promise((resolve, reject) => {
      const query = `
        SELECT 
  ...
  getLogs() {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT log_id AS logID, log_date, cleaner_count, homeowner_count FROM logs ORDER BY log_date DESC;"; // Ensure column names match schema if needed
