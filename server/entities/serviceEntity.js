...
class ServiceEntity {
  ...
  getAllServices() {
    return new Promise((resolve, reject) => {
      db.query(
        ...
        s.shortlistCount,
        s.cleanerID,
        a.username FROM services s
        JOIN accounts a ON s.cleanerID = a.id`,
        (err, results) => {
  ...
  getServicesByCleanerId(cleanerID) {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT * FROM services WHERE cleanerID = ?",
        [cleanerID],
        (err, results) => {
          if (err) return reject(err);
          if (results.length === 0)
            return reject("No services found for this cleaner");
          resolve(results);
        }
      );
    });
  ...
  addService(category, cleanerID, price) {
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO services (category, cleanerID, price) VALUES (?, ?, ?)";
  ...
  updateService(serviceID, updatedData) {
    return new Promise((resolve, reject) => {
      ...
      const query =
        "UPDATE services SET category = ?, cleanerID = ?, price = ? WHERE serviceID = ?";
      db.query(
        query,
        [category, cleanerID, price, serviceID],
        (err, results) => {
          if (err) return reject(err);
  ...
  getServicesByHownerId(hownerID) {
    return new Promise((resolve, reject) => {
      const query = `SELECT s.serviceID,
        s.category,
        s.cleanerID,
        s.price,
        s.view,
    ...
    `;
      db.query(query, [hownerID], (err, results) => {
        if (err) return reject(err);
        if (results.length === 0)
          return reject("No services found for this homeowner");
        resolve(results);
      });
    });
  }
