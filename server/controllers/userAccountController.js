...
class CreateUserController {
  static async create(req, res) {
    ...
    if (!username || !password || !profile) {
      return res
        .status(400)
        .json({ error: "Username, password, and profile are required" });
    }

    try {
      const newUser = await userAccountEntity.addUser(
        username,
        password,
        profile,
        suspended
...
class UpdateUserController {
  static async update(req, res) {
    ...
    try {
      const updatedUser = await userAccountEntity.updateUser(
        Number(id),
        updatedData
      );
      res
      ...
      res
        .status(404)
        .json({ error: "User not found or failed to update", details: error });
    }
  }
}
