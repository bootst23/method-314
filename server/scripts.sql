...
-- do not run event for now
DELIMITER //

CREATE EVENT daily_user_count
ON SCHEDULE EVERY 1 DAY
STARTS CURRENT_DATE + INTERVAL 23 HOUR + INTERVAL 59 MINUTE + INTERVAL 59 SECOND
DO
BEGIN
  INSERT INTO logs (log_date, cleaner_count, homeowner_count)
  SELECT 
    CURRENT_DATE(),
    (SELECT COUNT(*) FROM users WHERE role = 'cleaner'),
    (SELECT COUNT(*) FROM users WHERE role = 'homeowner');
END //
...
SELECT * FROM services WHERE cleanerID = 1;
SELECT serviceID FROM shortlists WHERE hownerID = 1;
SELECT * FROM views;
SELECT * FROM shortlists;
SELECT * FROM profiles WHERE profileID = 4;
SELECT * FROM logs;
show tables;
